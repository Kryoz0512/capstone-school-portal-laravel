...
type Props = {
    ...
    filters: { grade_level_id: number | null; section_id: number | null; subject_id: number | null; school_year: string }
                                    ...
                                    <tbody className="divide-y divide-gray-200">
                                        {students.length > 0 ? students.map((student, index) => (
                                            <tr key={student.id} className={`hover:bg-gray-50 ${!isVisibleOnScreen(index) ? 'hidden print:table-row' : ''}`}>
                                                <td className="px-2 sm:px-4 py-3 sm:py-4 text-sm text-gray-900">{index + 1}</td>
